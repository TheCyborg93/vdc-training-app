...
function boardState(board: LiveBoardSnapshot) {
...
export default function LiveBoardManagement({
  trainingDayId,
  boards,
  unassignedPlayers,
  onChanged,
  ...
  const allPlayers = useMemo(
    () => [...boards.flatMap((board) => board.players), ...unassignedPlayers],
    [boards, unassignedPlayers],
  );
  const selectedPlayer =
    allPlayers.find((player) => player.id === selectedPlayerId) ?? null;
  const freeBoards = boards.filter(
    (board) => board.players.length === 0 && board.status !== "COMPLETED",
  ).length;
  ...
  async function move(playerId: number, targetBoardId: number | null) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          payload.error ?? "Spieler konnte nicht verschoben werden.",
        );
      }
      notify(payload.message ?? "Boardzuweisung aktualisiert.", {
        tone: "success",
      });
      ...
      notify("Umbesetzung nicht möglich", {
  ...
  return (
    <section
      className={`phase6-board-manager ${open ? "is-open" : ""}`}
      aria-label="Live Boardverwaltung"
    >
      <header className="phase6-board-manager-head">
        <div>
          ...
          <p>
            Per Drag & Drop oder Spieler auswählen und anschließend das Zielboard
            antippen.
          </p>
        </div>
        <div className="phase6-board-manager-summary">
          <span>
            <strong>{freeBoards}</strong> freie Boards
          </span>
          <span>
            <strong>{unassignedPlayers.length}</strong> auf der Bank
          </span>
          <button
            type="button"
            onClick={() => {
      ...
      {open && (
        <div className="phase6-board-manager-body">
          {selectedPlayer && (
            <div className="phase6-board-manager-selection">
          ...
          <div
            className={`phase6-board-bank ${
              selectedPlayerId !== null || dragPlayerId !== null
                ? "is-target"
                : ""
            }`}
            ...
            onClick={() => {
              if (selectedPlayerId !== null) chooseTarget(null);
            }}
          >
            <header>
              <div>
                <span>SPIELERBANK</span>
                <h3>Nicht zugewiesen</h3>
              </div>
              <strong>{unassignedPlayers.length}</strong>
            </header>
            <div className="phase6-board-player-list">
              ...
              {unassignedPlayers.length === 0 && (
                <p>Alle anwesenden Spieler sind einem Board zugewiesen.</p>
              )}
            </div>
          </div>
          ...
          <div className="phase6-board-manager-grid">
            {boards.map((board) => {
              ...
              return (
                <article
                  key={board.id}
                  className={`phase6-board-slot status-${board.status.toLowerCase()} ${
                    selectedPlayerId !== null && !locked ? "is-target" : ""
                  } ${locked ? "is-locked" : ""}`}
                  onDragOver={(event) => {
                  ...
                  onDrop={(event) => {
                    event.preventDefault();
                    if (!locked) chooseTarget(board.board.id);
                  }}
                  onClick={() => {
                    if (selectedPlayerId !== null && !locked) {
                      chooseTarget(board.board.id);
                    }
                  }}
                >
                  <header>
                    <div>
                      ...
                      <small>{board.board.location ?? "Trainingsbereich"}</small>
                    </div>
                  ...
                  <div className="phase6-board-player-list">
                    ...
                    {board.players.length === 0 && (
                      <p>Board frei – Spieler hier ablegen.</p>
                    )}
                  </div>
                  {selectedPlayerId !== null && !locked && (
                    <div className="phase6-board-drop-hint">Hier zuweisen</div>
                  )}
                </article>
              );
            })}
