...
export default function ExerciseResultInput({ resultType, exerciseName, completionMode = "ENGINE_DEFAULT", completionValue, state, disabled = false, onSubmit }: Props) {
      ...
      <div className="visit-entry">
        ...
        {kind === "BOB27" && (
          <div className="bob27-scoreboard" aria-live="polite">
            <div><small>Aktueller Punktestand</small><strong>{currentScore}</strong><span>Startwert 27</span></div>
            <div><small>Aktuelles Ziel</small><strong>{state?.target ?? "D1"}</strong><span>Wert pro Treffer: +{doubleValue}</span></div>
            <div><small>Aufnahme</small><strong>{state?.visit ?? 1}</strong><span>{state?.dartsThrown ?? 0} Darts geworfen</span></div>
          </div>
        )}
        ...
        <div className="result-button-grid bob27-buttons">
          {[0, 1, 2, 3].map((hits) => {
            ...
            return <button disabled={disabled || remainingSeconds === 0} key={hits} onClick={() => void onSubmit({ hits })}><strong>{hits}</strong><span>Treffer</span>{kind === "BOB27" && <em>Neuer Stand: {preview}</em>}</button>;
          })}
        </div>
