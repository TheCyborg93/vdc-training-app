...
function payloadSummary(payload: Record<string, unknown>) {
  ...
  if (typeof payload.checkout === "boolean") {
    return payload.checkout ? `Checkout in ${payload.dartsUsed ?? "–"} Darts` : "Fehlversuch gespeichert";
  }
...
function targetCopy(kind: string, target: string | undefined, exerciseName: string) {
  if (target) {
    return kind.includes("CHECKOUT") || kind === "GAME_121" || kind === "CATCH_40"
      ? `Checkout ${target}`
      : `Treffer auf ${target}`;
  }
...
function modeLabel(mode: string, kind: string) {
  ...
  } as Record<string, string>)[mode] ?? "Ergebnis";
}
...
export default function ExerciseResultInput({
  ...
  const catch40Target = Math.max(
    40,
    Math.min(170, Math.trunc(Number(state?.target ?? config.target ?? config.startTarget ?? 40))),
  );
  ...
  const scoreInvalid = numericScore !== null && (
    ...
    || numericScore < 0
    || (kind === "CATCH_40"
      ? numericScore > catch40MaxScore
      : numericScore > 180 || IMPOSSIBLE_SCORES.has(numericScore))
  );
  ...
  async function dispatch(payload: Record<string, unknown>, reset = true) {
    ...
    try {
      ...
      window.setTimeout(() => setSuccessPulse(false), 480);
  ...
  const limitCurrent = completionMode === "VISIT_LIMIT"
    ? usedVisits
    : completionMode === "DART_LIMIT"
      ? state?.dartsThrown ?? 0
      : null;
  const progressLabel = completionMode === "TIME_LIMIT"
    ? "Verbleibende Zeit"
    : completionMode === "DART_LIMIT"
      ? "Darts"
      : "Aufnahmen";
  const progressValue = completionMode === "TIME_LIMIT"
    ? formatTime(remainingSeconds ?? Number(completionValue ?? 0) * 60)
    : `${limitCurrent ?? 0} / ${completionValue ?? 0}`;
  ...
  if (definition.inputMode === "HITS") {
    module = (
      <div className="vdcx-choice-grid">
        {Array.from({ length: definition.dartsPerVisit + 1 }, (_, hits) => (
          <button type="button" disabled={locked} key={hits} onClick={() => void dispatch({ hits })}>
            <strong>{hits}</strong><span>{hits === 0 ? "Keine Treffer" : "Treffer"}</span>
          </button>
        ))}
      </div>
    );
    ...
    module = (
      <>
        <div className="vdcx-segment-grid">
          {[["Single", single, setSingle], ["Doppel", double, setDouble], ["Treble", triple, setTriple]].map(([label, value, setter]) => (
            <div className="vdcx-segment-control" key={String(label)}>
              <span>{String(label)}</span>
              <div>
                <button type="button" disabled={locked || Number(value) <= 0} onClick={() => (setter as (n: number) => void)(Math.max(0, Number(value) - 1))}>−</button>
                <strong>{Number(value)}</strong>
                <button type="button" disabled={locked || usedDarts >= definition.dartsPerVisit} onClick={() => (setter as (n: number) => void)(Number(value) + 1)}>+</button>
              </div>
            </div>
          ))}
        </div>
        <div className={`vdcx-balance ${usedDarts === definition.dartsPerVisit ? "is-ready" : usedDarts > definition.dartsPerVisit ? "is-error" : ""}`}>
          <span>Erfasste Treffer</span><strong>{usedDarts} / {definition.dartsPerVisit}</strong>
        </div>
        <Guidance tone={usedDarts === 0 ? "info" : "neutral"}>
          {usedDarts === 0 ? "0 Treffer wird als vollständige Aufnahme gespeichert." : "Segmenttreffer dieser Aufnahme speichern."}
        </Guidance>
        <PrimaryButton disabled={locked || usedDarts > definition.dartsPerVisit} busy={submitting} onClick={() => void dispatch({ single, double, triple, hits: usedDarts })}>
          Aufnahme speichern
        </PrimaryButton>
      </>
    );
    ...
    currentTargetCopy = `${inRule} In · ${outRule} Out`;
    ...
    module = (
      <>
        <ScoreInput ref={scoreRef} value={score} onChange={setScore} disabled={locked} onSubmit={() => void submitScore({ checkout: checkoutType !== "NONE", checkoutType, doubleIn })} />
        {inRule === "DOUBLE" && !state?.opened && (
          <Toggle active={doubleIn} disabled={locked} onClick={() => setDoubleIn(!doubleIn)}>Mit Doppel eröffnet</Toggle>
        )}
        <div className="vdcx-toggle-grid">
          <Toggle active={checkoutType === "NONE"} disabled={locked} onClick={() => setCheckoutType("NONE")}>Kein Checkout</Toggle>
          {outRule === "SINGLE" && <Toggle active={checkoutType === "SINGLE"} disabled={locked} onClick={() => setCheckoutType("SINGLE")}>Single Out</Toggle>}
        ...
        <Guidance tone={scoreInvalid ? "error" : bust ? "warning" : nextRest !== null ? "info" : "neutral"}>
          {scoreInvalid ? "Bitte einen möglichen Score zwischen 0 und 180 eingeben." : bust ? "Bust: Der vorherige Rest bleibt bestehen." : nextRest !== null ? `Voraussichtlicher Rest: ${Math.max(0, nextRest)}` : "Score eingeben oder Schnellwert wählen."}
        </Guidance>
        <PrimaryButton disabled={locked || numericScore === null || scoreInvalid} busy={submitting} onClick={() => void submitScore({ checkout: checkoutType !== "NONE", checkoutType, doubleIn })}>Aufnahme speichern</PrimaryButton>
      </>
    );
    ...
    currentTargetCopy = `Checkout ${state?.target ?? "–"}`;
    module = (
      <>
        <div className="vdcx-mini-stats">
          <div><span>Finish</span><strong>{state?.target ?? "–"}</strong></div>
          <div><span>Darts im Versuch</span><strong>{state?.attemptDarts ?? 0} / {maxDarts}</strong></div>
          <div><span>Erfolge</span><strong>{state?.successes ?? 0}</strong></div>
        </div>
        {kind === "GAME_121" && <ScoreInput ref={scoreRef} value={score} onChange={setScore} disabled={locked} onSubmit={() => void submitScore({ checkout, dartsUsed })} />}
        <div className="vdcx-toggle-grid">
          <Toggle active={!checkout} disabled={locked} onClick={() => setCheckout(false)}>Nicht geschafft</Toggle>
          <Toggle active={checkout} success disabled={locked} onClick={() => setCheckout(true)}>Checkout geschafft</Toggle>
        </div>
        <DartsSelector value={dartsUsed} max={maxDarts} disabled={locked} onChange={setDartsUsed} />
        <PrimaryButton disabled={locked || (kind === "GAME_121" && (numericScore === null || scoreInvalid))} busy={submitting} onClick={() => void (kind === "GAME_121" ? submitScore({ checkout, dartsUsed }) : dispatch({ checkout, dartsUsed }))}>Versuch speichern</PrimaryButton>
      </>
    );
    ...
    currentTargetCopy = "Marken und Zusatzpunkte";
    module = (
      <>
        <label className="vdcx-field"><span>Ziel</span><select disabled={locked} value={target || String(state?.target ?? targets[0])} onChange={(event) => setTarget(event.target.value)}>{targets.map((item) => <option key={String(item)}>{String(item)}</option>)}</select></label>
        <div className="vdcx-choice-grid is-four">{[0, 1, 2, 3].map((value) => <button type="button" disabled={locked} key={value} className={marks === value ? "is-active" : ""} onClick={() => setMarks(value)}><strong>{value}</strong><span>{value === 0 ? "Keine Marken" : "Marken"}</span></button>)}</div>
        <label className="vdcx-field"><span>Zusatzpunkte</span><input disabled={locked} type="number" min="0" inputMode="numeric" value={points} onChange={(event) => setPoints(Math.max(0, Number(event.target.value)))} /></label>
        <PrimaryButton disabled={locked} busy={submitting} onClick={() => void dispatch({ target: target || state?.target || String(targets[0]), marks, points })}>Aufnahme speichern</PrimaryButton>
      </>
    );
  } else if (definition.inputMode === "KILLER") {
    const lives = state?.lives ?? Number(config.startLives ?? 3);
    currentValue = lives;
    valueLabel = "Leben";
    currentTarget = state?.target ?? "Gegner";
    currentTargetCopy = "Lebensstand verändern";
    ...
    currentTargetCopy = "Aktuelles Spielfeld";
    ...
    valueLabel = "Punkte";
    currentTarget = catch40Target;
    currentTargetCopy = `Catch 40 · ${catch40Darts} Darts`;
    const reached = numericScore !== null && numericScore >= catch40Target;
    module = (
      <>
        <div className="vdcx-mini-stats"><div><span>Aktuelles Ziel</span><strong>{catch40Target}</strong></div><div><span>Dartlimit</span><strong>{catch40Darts} Darts</strong></div><div><span>Maximaler Score</span><strong>{catch40MaxScore}</strong></div></div>
        <ScoreInput ref={scoreRef} value={score} onChange={setScore} disabled={locked} onSubmit={() => void submitScore({ target: catch40Target, dartsAllowed: catch40Darts, reachedTarget: reached })} max={catch40MaxScore} quickScores={[0, 40, 60, 80, 100, 120, 180]} placeholder={`0–${catch40MaxScore}`} />
        ...
        <PrimaryButton disabled={locked || numericScore === null || scoreInvalid} busy={submitting} onClick={() => void submitScore({ target: catch40Target, dartsAllowed: catch40Darts, reachedTarget: reached })}>Catch-40-Ergebnis speichern</PrimaryButton>
      </>
    );
  } else {
    currentValue = state?.score ?? 0;
    ...
    currentTargetCopy = definition.inputMode === "SCORE" ? "Punkte dieser Aufnahme" : currentTargetCopy;
  ...
  return (
    <section className={`vdcx-shell ${submitting ? "is-saving" : ""} ${successPulse ? "is-success" : ""} ${paused ? "is-paused" : ""}`} aria-busy={submitting} data-engine-kind={kind}>
...
function Metric({ label, value, caption, target = false }: { label: string; value: string | number; caption: string; target?: boolean }) {
  return <article className={`vdcx-metric ${target ? "is-target" : ""}`}><span>{label}</span><strong key={String(value)}>{value}</strong><small>{caption}</small></article>;
}

function PrimaryButton({ disabled, busy, onClick, children }: { disabled: boolean; busy: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" className="vdcx-primary" disabled={disabled} onClick={onClick}><span>{busy ? "Speichert …" : children}</span><b>→</b></button>;
}

function Toggle({ active, success = false, disabled, onClick, children }: { active: boolean; success?: boolean; disabled: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" className={`vdcx-toggle ${active ? "is-active" : ""} ${success && active ? "is-success" : ""}`} disabled={disabled} onClick={onClick}>{children}</button>;
}

function Guidance({ tone, children }: { tone: "neutral" | "info" | "warning" | "error"; children: React.ReactNode }) {
  return <div className={`vdcx-guidance is-${tone}`}>{children}</div>;
}

function DartsSelector({ value, max, disabled, onChange }: { value: number; max: number; disabled: boolean; onChange: (n: number) => void }) {
  return <div className="vdcx-darts"><span>Verwendete Darts</span><div>{Array.from({ length: max }, (_, index) => index + 1).map((number) => <button type="button" disabled={disabled} className={value === number ? "is-active" : ""} key={number} onClick={() => onChange(number)}>{number}</button>)}</div></div>;
}

const ScoreInput = forwardRef<HTMLInputElement, {
  value: string;
  ...
  quickScores?: number[];
  placeholder?: string;
}>(function ScoreInput({ value, onChange, disabled, onSubmit, max = 180, quickScores = QUICK_SCORES, placeholder = "0–180" }, ref) {
  return (
    <div className="vdcx-score">
      <label><span>Score</span><input ref={ref} disabled={disabled} autoFocus type="number" inputMode="numeric" min="0" max={max} value={value} onChange={(event) => onChange(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && value !== "" && !disabled) { event.preventDefault(); onSubmit(); } }} placeholder={placeholder} /></label>
      <div>{quickScores.filter((number) => number <= max).map((number) => <button type="button" disabled={disabled} className={value === String(number) ? "is-active" : ""} key={number} onClick={() => onChange(String(number))}>{number}</button>)}</div>
      <small>0 ist eine gültige Aufnahme · Enter speichert</small>
    </div>
  );
});
