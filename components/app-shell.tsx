...
const pageNames: Record<string, string> = {
  "/": "Dashboard", "/training": "Trainingstag", "/heimtraining": "Heimtraining", "/statistik": "Statistik",
  "/trainer": "Trainer-Dashboard", "/trainer/trainingstag": "Trainingstag", "/trainer/trainingsplaene": "Trainingspläne",
  "/trainer/heimtraining": "Heimtraining erstellen", "/trainer/uebungen": "Übungskatalog", "/trainer/live": "Live Center",
  "/trainer/spieler": "Spieler", "/trainer/boards": "Boards", "/trainer/statistiken": "Statistiken V2",
  "/trainer/ai-coach": "AI Coach", "/trainer/archiv": "Trainingsarchiv", "/trainer/system": "Systeminformationen",
};
...
function Icon({ name }: { name: IconName }) {
  ...
  const paths: Record<IconName, ReactNode> = {
    ...
    system: <><circle cx="12" cy="12" r="3"/><path d="M19 15l2 2-4 4-2-2M9 20H5v-4M5 8V4h4M15 4h4v4"/></>,
...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const checkHealth = async () => {
      ...
      try {
        ...
        if (!disposed) setHealth(response.ok && data.ok ? "online" : "offline");
  ...
  return <div className={`vdc-app ${trainerMode ? "is-trainer" : "is-player"} ${focusMode ? "is-focus-route" : ""} ${collapsed ? "is-collapsed" : ""}`}>
    <aside className="vdc-sidebar">
      <div className="vdc-sidebar-head">
        <Link href={trainerMode ? "/trainer" : "/"} className="vdc-brand" aria-label="Vestischer Dart Club Startseite"><span className="vdc-brand-mark" aria-hidden="true"><i/><i/><i/></span><span className="vdc-brand-copy"><strong>Vestischer</strong><b>Dart Club e.V.</b><small>Training OS</small></span></Link>
      ...
      <nav className="vdc-nav" aria-label={trainerMode ? "Trainernavigation" : "Spielernavigation"}>{groups.map((group) => <div className="vdc-nav-group" key={group.label}><small>{group.label}</small>{group.items.map((item) => { const active = item.href === "/" ? pathname === "/" : pathname === item.href || pathname.startsWith(`${item.href}/`); return <Link key={item.href} href={item.href} className={active ? "is-active" : ""} title={collapsed ? item.label : undefined}><span className="vdc-nav-icon"><Icon name={item.icon}/>{item.icon === "live" && <i className="vdc-live-dot"/>}</span><b>{item.label}</b></Link>; })}</div>)}</nav>
