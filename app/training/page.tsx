...
export default function LiveTrainingPage() {
  ...
  async function loadTraining(signal?: AbortSignal) {
    try {
      ...
      if (!response.ok) throw new Error(data.error ?? "Training konnte nicht geladen werden.");
      setTraining(data);
      setBoardId((current) => current ?? data?.assignments?.[0]?.boardId ?? null);
    } catch (error) {
      if (signal?.aborted) return;
      const text = error instanceof Error ? error.message : "Training konnte nicht geladen werden.";
      setMessage(text);
      notify("Training nicht verfügbar", { message: text, tone: "error" });
    }
  }
  ...
  useEffect(() => {
    ...
    const refresh = () => { if (document.visibilityState === "visible" && !saving && !report) void loadTraining(); };
  ...
  const infoMetrics = currentPlanExercise && progress && session ? competitionInfo(currentExerciseState, currentPlanExercise.exercise.name, session.board.name, progress.exerciseIndex, training?.trainingPlan.exercises.length ?? 1) : [];

  async function startTraining() {
    if (!training || !boardId) return;
    const boardName = boards.find((board) => board.id === boardId)?.name ?? "diesem Board";
    const accepted = await confirm({
      title: `Training an ${boardName} starten?`,
      message: `Die Spielerreihenfolge wird zufällig ausgelost. Danach beginnt die erste Übung direkt an ${boardName}.`,
      confirmLabel: "Training starten",
    ...
    try {
      ...
      notify("Training gestartet", { message: `${boardName} ist jetzt aktiv.`, tone: "success" });
      await loadTraining();
    } catch (error) {
      const text = error instanceof Error ? error.message : "Start fehlgeschlagen.";
      setMessage(text);
      notify("Training konnte nicht starten", { message: text, tone: "error" });
  ...
  async function boardControl(action: "pause" | "resume") {
    ...
    try {
      const response = await fetch("/api/trainer/live/control", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ boardSessionId: session.id, action }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error ?? "Board konnte nicht gesteuert werden.");
      notify(action === "pause" ? "Training pausiert" : "Training fortgesetzt", { message: action === "pause" ? "Der aktuelle Stand wurde gespeichert." : "Die Ergebniseingabe ist wieder aktiv.", tone: "success" });
      await loadTraining();
    } catch (error) {
      const text = error instanceof Error ? error.message : "Boardsteuerung fehlgeschlagen.";
      setMessage(text);
      notify("Boardsteuerung fehlgeschlagen", { message: text, tone: "error" });
  ...
  async function undoLastResult() {
    ...
    const accepted = await confirm({
      ...
      message: "Die zuletzt gespeicherte Aufnahme wird entfernt und der vorherige Spielstand wiederhergestellt.",
    ...
    try {
      ...
      notify("Aufnahme zurückgenommen", { message: "Der vorherige Trainingsstand wurde wiederhergestellt.", tone: "success" });
      await loadTraining();
    } catch (error) {
  ...
  async function saveResult(value: Record<string, unknown>) {
    ...
    try {
      ...
      if (data.completed && data.report) {
        ...
        notify("Training abgeschlossen", { message: "Der Abschlussbericht ist jetzt verfügbar.", tone: "success" });
  ...
  if (report) return <div className="training-focus-overlay"><TrainingReportView report={report} onClose={() => setReport(null)} /></div>;
  ...
  if (focusActive && currentPlanExercise && currentPlayer && progress && session) return (
    <div className="training-focus-overlay result-grid-overlay">
      <main className="training-result-shell">
        <header className="training-result-header"><div><small>{session.board.name} · Übung {progress.exerciseIndex + 1} von {training.trainingPlan.exercises.length}</small><strong>{currentPlanExercise.exercise.name}</strong></div><span>{progressPercent}%</span></header>
        <section className="training-result-grid">
          <article className="result-grid-score"><small>Aktueller Punktestand</small><strong>{currentExerciseState?.score ?? "–"}</strong><span>Aufnahme {currentExerciseState?.visit ?? 1}</span></article>
          <article className="result-grid-target"><small>Aktuelles Ziel</small><strong>{currentExerciseState?.target ?? currentPlanExercise.exercise.name}</strong><span>{currentExerciseState?.dartsThrown ?? 0} Darts gespielt</span></article>
          <article className="result-grid-player"><small>Wer ist dran?</small><strong>{currentPlayer.displayName}</strong><span>Danach: {nextPlayer?.displayName ?? "–"}</span></article>
          <article className="result-grid-free"><div className="competition-live-metrics">{infoMetrics.map((metric) => <div key={metric.label}><small>{metric.label}</small><strong>{metric.value}</strong>{metric.detail && <span>{metric.detail}</span>}</div>)}</div><div className="result-grid-progress"><span style={{ width: `${progressPercent}%` }} /></div></article>
          <section className="result-grid-engine">{session.status === "RUNNING" ? <ExerciseResultInput resultType={currentPlanExercise.exercise.resultType} exerciseName={currentPlanExercise.exercise.name} state={currentExerciseState} disabled={saving} onSubmit={saveResult} /> : <div className="result-grid-paused"><strong>Training pausiert</strong><span>Fortsetzen, um wieder Ergebnisse einzutragen.</span></div>}</section>
          <footer className="result-grid-undo competition-actions"><button className="button secondary" disabled={saving} onClick={() => void undoLastResult()}>Letzte Aufnahme rückgängig</button><button className="button" disabled={saving} onClick={() => void boardControl(session.status === "PAUSED" ? "resume" : "pause")}>{session.status === "PAUSED" ? "Training fortsetzen" : "Training pausieren"}</button>{message && <p className="form-message">{message}</p>}</footer>
        </section>
      </main>
    </div>
  );
  ...
  return (
    <main className={`${styles.root} dashboard-page`}>
      ...
      <section className="live-training-layout">
        <aside className="card admin-form"><label>Board bestätigen<select value={boardId ?? ""} onChange={(event) => { setBoardId(Number(event.target.value)); setReport(null); }}>{boards.map((board) => <option key={board.id} value={board.id}>{board.name}</option>)}</select></label><div className="board-confirmation"><small>Ausgewähltes Board</small><strong>{boards.find((board) => board.id === boardId)?.name}</strong><span>{boardPlayers.length} Spieler eingeteilt</span></div><button className="button full" disabled={starting || session?.status === "RUNNING" || session?.status === "COMPLETED"} onClick={() => void startTraining()}>{session?.status === "COMPLETED" ? "Training beendet" : starting ? "Startet …" : "Training starten"}</button>{message && <p className="form-message">{message}</p>}</aside>
        <section className="club-panel"><div className="section-heading"><div><span className="eyebrow">Board-Zuteilung</span><h2>{boards.find((board) => board.id === boardId)?.name}</h2></div></div><div className="live-player-list">{boardPlayers.map((item, index) => <article className="live-player-card" key={item.player.id}><span>{index + 1}</span><strong>{item.player.displayName}</strong><small>Bereit</small></article>)}</div></section>
      </section>
    </main>
  );
}
