...
export default function PlayerProfilePage() {
  ...
  return <main className="dashboard-page player-profile-v3">
    <section className="dashboard-heading"><div><div className="eyebrow">Phase 7.1 · Spieleranalyse</div><h1>{player.displayName}</h1><p>{player.firstName} · Vereins- und Heimtraining gemeinsam ausgewertet.</p></div><div className="actions"><select value={periodDays} onChange={(event) => setPeriodDays(Number(event.target.value))} aria-label="Analysezeitraum"><option value={30}>30 Tage</option><option value={90}>90 Tage</option><option value={180}>180 Tage</option><option value={365}>365 Tage</option></select><Link className="button secondary" href="/trainer/spieler">Spielerübersicht</Link><Link className="button" href="/trainer/ai-coach">AI Coach</Link></div></section>
