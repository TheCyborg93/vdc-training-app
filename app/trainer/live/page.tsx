...
import { coachHealthPriority, getBoardCoachHealth } from "@/lib/live-training/health";
...
export default function TrainerLivePage() {
  ...
  }, [load, busyBoardId, bulkBusy, realtimeStatus]);
  ...
  async function runBulk(action: BulkAction) {
    ...
    setBulkBusy(true);
    try {
      await Promise.all(candidates.map(async (board) => {
        const response = await fetch("/api/trainer/live/control", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ boardSessionId: board.id, action }),
  ...
  return (
    <main className="phase6-live">
      ...
      <nav className="phase6-global-actions">
        <Link href="/trainer/live/coach">Coach View</Link>
