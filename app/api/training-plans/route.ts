...
export async function GET() {
  try {
    ...
    const withArchiveState = plans.map((plan) => ({
      ...plan,
      status: plan.trainingDays.some((day) => day.status === "COMPLETED") ? "ARCHIVED" : plan.status,
    }));
    return NextResponse.json(withArchiveState);
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const normalizedItems = items.map((item: { exerciseId?: unknown; durationMin?: unknown }, index: number) => ({ exerciseId: Number(item.exerciseId), durationMin: Number(item.durationMin), position: index + 1 }));
