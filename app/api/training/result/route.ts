...
export async function POST(request: Request) {
  try {
    ...
    if (!session || session.status !== "RUNNING") {
      return NextResponse.json({ error: "Diese Board-Sitzung läuft aktuell nicht." }, { status: 409 });
    }
    ...
    if (!currentPlanExercise || !currentPlayerId) {
      return NextResponse.json({ error: "Aktuelle Übung oder Spieler konnte nicht bestimmt werden." }, { status: 409 });
    }
    ...
    const nextProgress = completed
      ? progress
      : { order: nextOrder, exerciseIndex: nextExerciseIndex, playerIndex: nextPlayerIndex, roundNumber: progress.roundNumber + 1 };
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.boardSession.update({
        where: { id: session.id },
        data: completed
          ? { status: "COMPLETED", completedAt: new Date(), currentExerciseId: null }
          : { currentExerciseId: nextExercise?.exerciseId ?? null, randomOrderJson: nextProgress },
      });

      if (completed) {
        const openSessions = await tx.boardSession.count({
          where: { trainingDayId: session.trainingDayId, status: { not: "COMPLETED" } },
        });
        if (openSessions === 0) {
    ...
    return NextResponse.json({ completed, nextProgress });
  } catch (error) {
